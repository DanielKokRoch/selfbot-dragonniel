...
module.exports = async (conn, msg, m, setting, store) => {
  try {
    ...
    const isOwner = [`${setting.ownerNumber}@s.whatsapp.net`, "62812177794270@s.whatsapp.net"].includes(sender) ? true : false
    ...
    const unlisted = command.startsWith(prefix)
    ...
    const authorWm = "SGC BOT by HilmySakti"
      ...
      conn.sendMessage(from, {
        ...
        contextInfo: {
          "externalAdReply":
          {
            ...
            sourceUrl: 'https://hilmysakti.xyz'
          }
        }
      },
        {
    ...
    const adOwner = async (quo) => {
      conn.sendMessage(from, {
        text: `https://api.whatsapp.com/send/?phone=${setting.ownerNumber}&text=hlo+bwang+:>&type=phone_number&app_absent=0`,
        contextInfo: {
          "externalAdReply":
          {
            showAdAttribution: true,
            ...
            sourceUrl: `https://api.whatsapp.com/send/?phone=${setting.ownerNumber}&text=hlo+bwang+:>&type=phone_number&app_absent=0`
          }
        }
      },
        {
    ...
    switch (command) {
      ...
      case 'menu': case 'help':
        if (!q) {
          ...
          cptn += `• ${prefix}hidetag\n`
          cptn += `• ${prefix}ssweb\n\n`
          cptn += `_Create by HilmySakti_\n_Since 01-12-2020_`
          adReply2(cptn, authorWm, setting.botName, fstatus)
...
`
      ...
      case 'grupbot':
      case 'groupbot':
        conn.sendMessage(from, { text: `${authorWm}\n${setting.group.link}` }, { quoted: fstatus })
        break
...
`
      ...
      default:
        if (unlisted) {
          if(isOwner) {
              adReply(`Ara~ Hilmy-kun cmd ${prefix + command} nggak ada di menu.`, '*UNLISTED CMD*')
          } else {
              adReply(`Maaf ${pushname}, tapi cmd ${prefix + command} tidak terdaftar dalam menu!.`, '*CMD UNLISTED*')
          }
      }
    }
  } catch (err) {
